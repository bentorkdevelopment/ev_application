...
// ─────────────────────────────────────────────────────────────────────────────
export default function TestScreen({ navigation, isOverlay = false }) {
                    ...
                    {isLoadingPlaces ? (
                        ...
                        <>
                            <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingHorizontal: 20, gap: 12 }}>
                                {nearbyPlaces.filter(p => p.type === 'Coffee' || p.type === 'Restaurant' || p.type === 'Cafe').slice(0, 5).map((place) => (
                                    <SquareCafeCard key={place.id} place={place} />
                                ))}
                            </ScrollView>
    ...
    return (
        <View style={[styles.rootWrapper, isOverlay && { backgroundColor: 'transparent' }]} pointerEvents={isOverlay ? 'box-none' : 'auto'}>
            ...
            {!isOverlay && (
                <View style={[StyleSheet.absoluteFill, { paddingTop: insets.top + 20, paddingHorizontal: 20 }]}>
                {/* Greeting & Header Stats */}
                <View style={{ marginBottom: 24, marginTop: 10, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                    <View>
                        <Text style={{ color: '#aaa', fontSize: 13, fontWeight: '600', letterSpacing: 1.2, textTransform: 'uppercase' }}>
                            {getDynamicGreeting()}, {userName}
                        </Text>
                        <Text style={{ color: '#fff', fontSize: 26, fontWeight: '800', marginTop: 4, letterSpacing: -0.5 }}>Welcome!</Text>
                    </View>
                    <View style={{ alignItems: 'flex-end', gap: 4, marginTop: 2 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                            <Sun size={14} color="#FBBC04" />
                            <Text style={{ color: '#E0E0E0', fontSize: 14, fontWeight: '600' }}>
                                {weather ? `${weather.temp}°C · ${weather.condition}` : '28°C · Sunny'}
                            </Text>
                        </View>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                            <Wind size={14} color="#39E29B" />
                            <Text style={{ color: '#aaa', fontSize: 12, fontWeight: '500' }}>
                                {aqi ? `${aqi.aqi} AQI · ${aqi.category}` : '87 AQI · Moderate'}
                            </Text>
                        </View>
                    </View>
                </View>
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 150 }}>
                    ...
                    {/* Suggested Card */}
                    <View style={[styles.googleCard, { paddingVertical: 16, marginBottom: 32, height: 165 }]}>
                        ...
                        <ScrollView
                            ...
                            onScroll={(e) => {
                                ...
                                const index = Math.round(x / (SCREEN_W - 72));
                                if (activeSuggestedTab !== index && index >= 0 && index <= 2) {
                                    setActiveSuggestedTab(index);
                                }
                            }}
                            style={{ flex: 1 }}
                        >
                            {/* Slide 1 */}
                            <View style={{ width: SCREEN_W - 72, paddingHorizontal: 16, justifyContent: 'center' }}>
                                <View style={{ gap: 12 }}>
                                    <Text style={{ color: '#aaa', fontSize: 13, fontWeight: '500' }}>You have 3 tasks due today and 1 upcoming meeting.</Text>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: 'rgba(52,168,83,0.1)', padding: 12, borderRadius: 12 }}>
                                        <Video size={20} color="#34A853" />
                                        <View style={{ flex: 1 }}>
                                            <Text style={{ color: '#E0E0E0', fontSize: 14, fontWeight: '600' }}>Weekly Standup</Text>
                                            <Text style={{ color: '#aaa', fontSize: 12, marginTop: 2 }}>Joining in 15 mins</Text>
                                        </View>
                                        <TouchableOpacity style={{ backgroundColor: '#39E29B', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8 }}>
                                            <Text style={{ color: '#000', fontSize: 13, fontWeight: '700' }}>Join</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                            </View>
                            ...
                            <View style={{ width: SCREEN_W - 72, paddingHorizontal: 16, justifyContent: 'center' }}>
                                <View style={{ gap: 12 }}>
                                    <Text style={{ color: '#aaa', fontSize: 13, fontWeight: '500' }}>Design reviews require your attention.</Text>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: 'rgba(234,67,53,0.1)', padding: 12, borderRadius: 12 }}>
                                        <Mail size={20} color="#EA4335" />
                                        <View style={{ flex: 1 }}>
                                            <Text style={{ color: '#E0E0E0', fontSize: 14, fontWeight: '600' }}>UI Notes</Text>
                                            <Text style={{ color: '#aaa', fontSize: 12, marginTop: 2 }}>From Design Team</Text>
                                        </View>
                                        <TouchableOpacity style={{ backgroundColor: '#39E29B', paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8 }}>
                                            <Text style={{ color: '#000', fontSize: 13, fontWeight: '700' }}>Read</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                            </View>
                            ...
                            <View style={{ width: SCREEN_W - 72, paddingHorizontal: 16, justifyContent: 'center' }}>
                                <View style={{ gap: 12 }}>
                                    <Text style={{ color: '#aaa', fontSize: 13, fontWeight: '500' }}>Your schedule is clear for tomorrow morning.</Text>
                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: 'rgba(66,133,244,0.1)', padding: 12, borderRadius: 12 }}>
                                        <Calendar size={20} color="#4285F4" />
                                        <View style={{ flex: 1 }}>
                                            <Text style={{ color: '#E0E0E0', fontSize: 14, fontWeight: '600' }}>Sprint Planning</Text>
                                            <Text style={{ color: '#aaa', fontSize: 12, marginTop: 2 }}>Tomorrow, 14:00 - 15:00</Text>
                                        </View>
                                    </View>
                                </View>
                            </View>
                        ...
                        <View style={{ flexDirection: 'row', justifyContent: 'center', marginTop: 12, gap: 6 }}>
                            {[0, 1, 2].map((index) => (
                                <View 
                                    key={index} 
                                    style={{
                                        width: activeSuggestedTab === index ? 16 : 6,
                                        height: 6,
                                        borderRadius: 3,
                                        backgroundColor: activeSuggestedTab === index ? '#39E29B' : '#555'
                                    }}
                                />
                            ))}
                        </View>
                    ...
                    {/* Office Section */}
                    <Text style={{ color: '#aaa', fontSize: 13, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 12 }}>Office</Text>
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 14, marginBottom: 24 }}>
                        {/* Gmail Card */}
                        <View style={[styles.googleCard, { flex: 1, minWidth: '45%' }]}>
                            <View style={[styles.googleIconBg, { backgroundColor: 'rgba(234,67,53,0.15)' }]}>
                                <Mail size={22} color="#EA4335" />
                            </View>
                            <Text style={styles.googleCardTitle}>Gmail</Text>
                            <Text style={styles.googleCardSub}>3 unread emails</Text>
                        </View>

                        {/* Calendar Card (Dynamic Popup) */}
                        <TouchableOpacity 
                            style={[styles.googleCard, { flex: 1, minWidth: '45%' }]}
                            activeOpacity={0.8}
                            onPress={() => setIsCalendarModalVisible(true)}
                        >
                            <View style={[styles.googleIconBg, { backgroundColor: 'rgba(66,133,244,0.15)' }]}>
                                <CalendarIcon size={22} color="#4285F4" />
                            </View>
                            <Text style={styles.googleCardTitle}>Calendar</Text>
                            <Text style={styles.googleCardSub}>
                                {calendarEvents.length > 0 ? `${calendarEvents.length} Events coming up` : 'View upcoming events'}
                            </Text>
                        </TouchableOpacity>

                        {/* Meet Card */}
                        <View style={[styles.googleCard, { flex: 1, minWidth: '45%' }]}>
                            <View style={[styles.googleIconBg, { backgroundColor: 'rgba(52,168,83,0.15)' }]}>
                                <Video size={22} color="#34A853" />
                            </View>
                            <Text style={styles.googleCardTitle}>Meet</Text>
                            <Text style={styles.googleCardSub}>Join in 15 mins</Text>
                        </View>

                        {/* Tasks Card */}
                        <View style={[styles.googleCard, { flex: 1, minWidth: '45%' }]}>
                            <View style={[styles.googleIconBg, { backgroundColor: 'rgba(251,188,4,0.15)' }]}>
                                <CheckSquare size={22} color="#FBBC04" />
                            </View>
                            <Text style={styles.googleCardTitle}>Tasks</Text>
                            <Text style={styles.googleCardSub}>5 pending items</Text>
                        </View>
                    </View>

                    {/* Personal / Media Section */}
                    <Text style={{ color: '#aaa', fontSize: 13, fontWeight: '700', letterSpacing: 1, textTransform: 'uppercase', marginBottom: 12 }}>Personal</Text>
                    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 14 }}>
                        {/* YouTube Card (Dynamic Popup) */}
                        <TouchableOpacity 
                            style={[styles.googleCard, { flex: 1, minWidth: '45%' }]}
                            activeOpacity={0.8}
                            onPress={() => setIsYouTubeModalVisible(true)}
                        >
                            <View style={[styles.googleIconBg, { backgroundColor: 'rgba(255,0,0,0.15)' }]}>
                                <Youtube size={22} color="#FF0000" />
                            </View>
                            <Text style={styles.googleCardTitle}>YouTube</Text>
                            <Text style={styles.googleCardSub}>
                                {youtubeVideos.length > 0 ? `${youtubeVideos.length} EV Videos Available` : 'Browse EV Reviews'}
                            </Text>
                        </TouchableOpacity>
                        ...
                        <View style={[styles.googleCard, { flex: 1, minWidth: '45%' }]}>
                            <View style={[styles.googleIconBg, { backgroundColor: 'rgba(66,133,244,0.15)' }]}>
                                <Users size={22} color="#4285F4" />
                            </View>
                            <Text style={styles.googleCardTitle}>People</Text>
                            <Text style={styles.googleCardSub}>Sarah's Birthday</Text>
                        </View>
                    </View>
                </ScrollView>
                </View>
                ...
                <>
                        ...
                        <View style={{ position: 'relative', width: 38, height: 38 }}>
                            ...
                            <Animated.View style={{ position: 'absolute', inset: 0, opacity: minimizeOpacity }}>
                                <TouchableOpacity 
                                    ...
                                    onPress={() => Animated.spring(pan, { toValue: 0, useNativeDriver: false, bounciness: 6 }).start()}
                                >
                                    <ChevronRight size={22} color="#fff" />
                                </TouchableOpacity>
                            </Animated.View>
                    ...
                    <View style={[styles.footer, { paddingBottom: insets.bottom + 10 }]}>
                        <TouchableOpacity 
                            ...
                            onPress={() => Animated.spring(pan, { toValue: Math.max(0, sheetHeightRef.current - 120), useNativeDriver: false, bounciness: 6 }).start()}
                        >
